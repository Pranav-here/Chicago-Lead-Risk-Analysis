...
def calculate_risk(row):
    ...
    if public_line == 'LEAD': risk += 0.7
    if private_line == 'LEAD': risk += 0.3
    return risk
...
try:
    ...
    gdf.plot(column='risk_score', ax=ax, markersize=10,
             legend=True, cmap='YlOrRd', alpha=0.7,
    ...
    print("Visualization successfully created: lead_risk_map.png")
...
merged.to_csv('data\\merged_data_for_maps.csv', index=False)
...
# [Keep all your initial imports and data processing code]

# After creating the static map visualization, add:

# Interactive Map Section
def create_interactive_map(gdf):
    """Create Folium interactive map with risk scores"""
    ...
    marker_cluster = MarkerCluster(name="Lead Risk Properties").add_to(m)

    for idx, row in gdf.iterrows():
        popup_html = f"""<h4>Address: {row['address']}</h4>
                        <b>Public Line:</b> {row['public_service_line_material_service']}<br>
                        <b>Private Line:</b> {row['private_service_line_material_service']}<br>
                        <b>Risk Score:</b> {row['risk_score']:.2f}<hr>
                        <b>Census Tract:</b> {row['tract_geoid']}<br>
        ...
        folium.CircleMarker(
            location=[row.geometry.y, row.geometry.x],  # Use geometry coordinates
            radius=6,
            color=colormap(row['risk_score']),
            fill=True,
            popup=folium.Popup(popup_html, max_width=300),
...
# Execute interactive map creation
try:
    interactive_map = create_interactive_map(gdf)
    print("Successfully created interactive_lead_risk_map.html")
except Exception as e:
    print(f"Failed to create interactive map: {str(e)}")
    raise